 ...
 * вопрос «что я получу».
 *
 * Половина программы ещё не утверждена: уроки есть у модулей 1 и 2, у
 * остальных четырёх — только название и результат (заказчик обещал их к 2–4
 * сентября). Такой блок не раскрывается вовсе: кнопки нет, вместо списка
 * стоит строка «Программа модуля утверждается» (Q13). Секция от этого не
 * выглядит поломанной и честно показывает состояние.
 *
        ...
        {hasLessons && (
          ...
          <span
            ...
            className={`relative flex size-6 items-center justify-center rounded-full border transition-colors duration-300 ${
              isOpen ? 'border-[#6AA0FF]/60 bg-[#6AA0FF]/14' : 'border-white/16'
            }`}
          >
            <span className="absolute h-px w-2.5 rounded-full bg-[#6AA0FF]" />
            <span
              className={`absolute h-2.5 w-px rounded-full bg-[#6AA0FF] transition-transform duration-300 ${
                isOpen ? 'scale-y-0' : ''
              }`}
            />
          </span>
  ...
  return (
    ...
    >
      ...
      {/* ── Статичная часть блока ────────────────────────────────────────
          ...
          раскрытия. У неутверждённых модулей над ним идёт строка о
          состоянии программы — она заменяет собой список уроков (Q13).
          У модуля 4 не утверждён и результат: там остаётся одна эта строка,
          и блок всё равно не выглядит сломанным. */}
      <div className="border-t border-white/8 bg-[rgba(6,10,16,.5)] px-3.5 py-3">
        ...
        {module.result && (
          <p className={`text-[12px] leading-[1.5] text-white/70 ${hasLessons ? '' : 'mt-2.5'}`}>
            ...
            <span className="font-display font-bold uppercase tracking-[.02em] text-[#6AA0FF]">
